<div class="relative flex w-full flex-wrap items-stretch">
    ...
    <input type="text" placeholder="Search here..." wire:model.live.debounce.300ms="search" class="border-0 px-3 py-3 placeholder-slate-300 text-slate-600 relative bg-white bg-white rounded text-sm shadow outline-none focus:outline-none focus:ring w-full pl-10" />
        ...
        @foreach($results as $group => $entries)
            ...
            <ul>
                @foreach($entries as $entry)
                    <li class="cursor-pointer flex items-center hover:bg-slate-100 block py-2 px-3 no-underline font-normal">
                        <a href="{{ $entry['linkTo'] }}">
                            @foreach($entry['fields'] as $name => $value)
                                <div class="text-sm text-slate-700">{{ $name }}: {{ $value }}</div>
                            @endforeach
                        </a>
                    </li>
                @endforeach
