    ...
    <div class="card-controls sm:flex">
        <div class="w-full sm:w-1/2">
            ...
            @if(file_exists(app_path('Http/Livewire/ExcelExport.php')))
                <livewire:excel-export model="Query" format="csv" />
                <livewire:excel-export model="Query" format="xlsx" />
                <livewire:excel-export model="Query" format="pdf" />
            @endif
            ...
            <table class="table table-index w-full">
                ...
                <tbody>
                    @forelse($queries as $query)
                        <tr>
                            <td>
                                <input type="checkbox" value="{{ $query->id }}" wire:model.live="selected">
                            </td>
                            <td>
                                {{ $query->id }}
                            </td>
                            ...
                            <td>
                                <div class="flex justify-end">
                                    @can('query_show')
                                        <a class="btn btn-sm btn-info mr-2" href="{{ route('admin.queries.show', $query) }}">
                                            {{ trans('global.view') }}
                                        </a>
                                    @endcan
                                    @can('query_edit')
                                        <a class="btn btn-sm btn-success mr-2" href="{{ route('admin.queries.edit', $query) }}">
                                            {{ trans('global.edit') }}
                                        </a>
                                    @endcan
                                    @can('query_delete')
                                        <button class="btn btn-sm btn-rose mr-2" type="button" wire:click="confirm('delete', {{ $query->id }})" wire:loading.attr="disabled">
                                            {{ trans('global.delete') }}
                                        </button>
                                    @endcan
                                </div>
